...
from PySide6.QtDataVisualization import QAbstract3DGraph, Q3DInputHandler, QScatterDataItem
...
class AxesInputHandler(Q3DInputHandler):
    ...
    def __init__(self, graph: QAbstract3DGraph, lineGraph: mplwidget.MplGraph , parent=None):
    ...
    def handleElementSelected(self, type):
        if type == QAbstract3DGraph.ElementAxisXLabel:
            ...
            if series:
                pointIndex = series.selectedItem()
            #     text += f"{point.x()}, {point.y()}"
            # text += ")"
            point = self._graph.seriesList()[0].dataProxy().itemAt(pointIndex)
            print(point.x(), point.y(), point.z())
            self._lineGraph.PlotLineGraph(mplwidget.PLOTDATA, point.z())
        else:
            self._state = InputState.StateNormal
